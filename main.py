...
api_request = requests.get(
    "https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest?start=1&limit=5&convert=NGN&CMC_PRO_API_KEY=3889ce63-c733-403d-8b7d-c41a2438b4ea")
...
for i in range(0, 4):
    for coin in coins:
        if api["data"][i]["symbol"] == coin["symbol"]:
            ...
            current_value = coin["amount_owned"] * api["data"][i]["quote"]["NGN"]["price"]
            pl_percoin = api["data"][i]["quote"]["NGN"]["price"] - coin["price_per_coin"]
            total_pl_coin = pl_percoin * coin["amount_owned"]

            total_pl = total_pl + total_pl_coin

            print(api["data"][i]["name"] + " - " + api["data"][i]["symbol"])
            print("Price - N{0:.3f}".format(api["data"][i]["quote"]["NGN"]["price"]))
            print("Number Of Coin:", coin["amount_owned"])
            print("Total Amount Paid:", "${0:.3f}".format(total_paid))
            print("Current Value:", "N{0:.3f}".format(current_value))
            print("P/L Per Coin:", "N{0:.3f}".format(pl_percoin))
            print("Total P/L With Coin:", "N{0:.3f}".format(total_pl_coin))
            print("----------------")

print("Total P/L For Portfolio:", "N{0:.3f}".format(total_pl))